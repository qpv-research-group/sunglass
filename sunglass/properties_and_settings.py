...
default_layer_properties = {
    ...
    "hole_diffusion_length": 40e-6,
}  # m

default_junction_properties = {
    "Sn": 1e10,
    "Sp": 1e10,
    "R_shunt": 1e14,
    "R_series": 0,
...
properties_junctions = {
    "Junction-PDD": ["Sn", "Sp", "R_shunt", "R_series"],
...
global_settings = [("T", 298, "K"), ("T_ambient", 298, "K")]

electrical_settings = [
    ("voltages", "0, 1.2, 100", "V, V, points"),
...
class popupWindow(tk.Toplevel):
    def __init__(self, master, text, value, units):
        ...
        ttk.Entry(frame, textvariable=self.new_value).grid(
            column=0, row=1, sticky=tk.NSEW
        )
        ttk.Label(frame, text=units).grid(column=1, row=1, sticky=tk.NSEW)
        ttk.Button(frame, text="Ok", command=self.cleanup).grid(
            column=0, row=2, sticky=tk.NSEW
        )
        ttk.Button(frame, text="Cancel", command=self.exit).grid(
    ...
    def load_global_settings(self):
        ...
        for value in global_settings:
            if value[0] in self.model["Global"].keys():
                self.settings_list.insert(
                    "",
                    "end",
                    text=value[0],
                    values=(self.model["Global"][value[0]], value[2]),
                    tags=("modified"),
                )
            else:
                self.settings_list.insert(
                    "", "end", text=value[0], values=(value[1], value[2])
                )
    ...
    def load_electrical_settings(self):
        ...
        for key in pdd_options.keys():
            if key in self.model["Electrical"].keys():
                self.settings_list.insert(
                    ...
                    "end",
                    text=key,
                    values=(self.model["Electrical"][key], ""),
                    tags=("modified"),
                )
            else:
                self.settings_list.insert(
                    "", "end", text=key, values=(pdd_options[key], "")
                )
        ...
        for key in asc_options.keys():
            if key in self.model["Electrical"].keys():
                ...
                self.settings_list.insert(
                    "", "end", text=key, values=(asc_options[key], "")
                )
        ...
        for value in optical_settings:
            if value[0] in self.model["Optical"].keys():
                self.settings_list.insert(
                    ...
                    "end",
                    text=value[0],
                    values=(self.model["Optical"][value[0]], value[2]),
                    tags=("modified"),
                )
            else:
                self.settings_list.insert(
        ...
        for key in rcwa_options.keys():
            if key in self.model["Optical"].keys():
                self.settings_list.insert(
                    ...
                    "end",
                    text=key,
                    values=(self.model["Optical"][key], ""),
                    tags=("modified"),
                )
            else:
                self.settings_list.insert(
                    "", "end", text=key, values=(rcwa_options[key], "")
                )
