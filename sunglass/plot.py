...
class IVPlot(object):
    ...
    def plot_dark_iv(self, data: pd.DataFrame) -> None:
        ...
        for name in data.columns.values:
            ...
            # Plotting junctions IV, when isolated and as part of the MJ structure
                ...
                self.axes.semilogy(
                    data["V"], abs(data[new_name]), color=color, label=label
                )

        # Finally the voltage drop due to series resistance and the total IV.
        self.axes.plot(
            data["V_Rs"], abs(data["J"]), "o--", fillstyle="none", label="Rs"
        )
        self.axes.semilogy(
            data["V"], abs(data["J"]), "k", linewidth=4, label="Total IV"
        )
    ...
    def plot_light_iv(self, data: pd.DataFrame, spectrum: LightSource) -> None:
        ...
        self.outputs["ff"] = self.outputs["mpp"] / (
            self.outputs["jsc"] * self.outputs["voc"]
        )
        ...
        for name in data.columns.values:
            ...
            # Plotting junctions IV, when isolated and as part of the MJ structure
                ...
                label = "Isolated {}".format(id)
                self.axes.plot(
                    sv * data["V"], sj * data[new_name], color=color, label=label
                )
        ...
        sv = 1 if np.interp(0, data["J"], data["V"]) >= 0 else -1
        self.axes.plot(
            sv * data["V_Rs"], sj * data["J"], "o--", fillstyle="none", label="Rs"
        )
        self.axes.plot(
            sv * data["V"], sj * data["J"], "k", linewidth=4, label="Total IV"
        )
...
class QEPlot(object):
    ...
    def plot_qe(self, data: pd.DataFrame, spectrum: LightSource) -> None:
        ...
        self.axes.fill_between(
            wl, y1, y2, color="yellow", alpha=0.5, linewidth=3, label="R"
        )
        ...
        for name in data.columns.values:
            if "A" in name:
                ...
                self.axes.fill_between(
                    wl, y1, y2, facecolor=colors[i], alpha=0.5, label=label
                )
                i += 1
            ...
            if spectrum is not None and name[:4] == "EQE":
                current = (
                    data[name].values
                    * spectrum.spectrum(x=wl, output_units="photon_flux_per_nm")[1]
                )
...
class BandstructurePlot(object):
    ...
    def __init__(self, data: pd.DataFrame, title: str = "") -> None:
        ...
        """
        self.data = data
        self.fig, (self.energy, self.carriers, self.gr) = plt.subplots(
            num=title, figsize=(8, 8), nrows=3, ncols=1, sharex="all"
        )
